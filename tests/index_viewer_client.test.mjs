...
test("the details render the chain, feeds and external ids, escaped", () => {
  ...
  const validity = {
    ...
    windows: [
      ...
      { start: "2026-10-01", end: "2026-12-31", feeds: 2 },
    ],
    best: { start: "2026-10-01", end: "2026-12-31", feeds: 2 },
  };
  const validityText = validityHtml(validity);
  assert.match(validityText, /2 dated feeds, 1 undated · valid 2026-08-01 to 2026-12-31 · best 2026-10-01 to 2026-12-31 \(2 feeds\)<\/p>/);
  assert.match(validityText, /<li>2026-08-01 to 2026-09-30: 1<\/li><li>2026-10-01 to 2026-12-31: 2<\/li>/);
  assert.equal(validityHtml({ feeds_dated: 0, feeds_undated: 2, windows: [], best: null }), '<p class="validity">0 dated feeds, 2 undated</p>');
  assert.equal(validityHtml(null), "");
  assert.match(detailsHtml({ properties: { ...record.properties, validity } }), /best 2026-10-01/);
...
test("feed and edge rows render names, dashes and the review flag", () => {
  ...
  assert.match(html, /^<tr data-id="f&lt;1"><td>f&lt;1<\/td><td>gtfs<\/td><td>—<\/td><td>FI<\/td><td>domestic<\/td><td>—<\/td><td>12<\/td><td>ok<\/td><td>3<\/td><td>—<\/td><td>—<\/td><td>—<\/td><td>2<\/td><td>0<\/td><td>1<\/td>/);
  ...
  assert.match(feedRowHtml({ ...feed, realtime: 1, category_primary: 4 }, feedColumns("category")), /<td>3<\/td><td>1<\/td><td>—<\/td><td>—<\/td><td>4<\/td><td>—<\/td>/);
...
test("a feed's details, served-places slice and legend follow its tiers", () => {
  ...
  const dated = { ...rankedFeed, properties: { ...rankedFeed.properties, service_start: "2026-09-01", service_end: "2026-09-14" } };
  assert.match(feedDetailsHtml(dated), /<p>valid 2026-09-01 to 2026-09-14<\/p>/);
